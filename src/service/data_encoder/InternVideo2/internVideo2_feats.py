...
class InternVideo2Feats:
    ...
    def create_npy(self, input_data_path:str, output_npy_dir):
        ...
        fn = self.config.get('num_frames', 1)
        ...
        if os.path.exists(input_images_path):
            images_paths = tqdm(os.listdir(input_images_path))

            for keyframe_images_path in images_paths:
                print(keyframe_images_path)
                # if keyframe_images_path in key:
                for txt_path in tqdm(glob.glob(os.path.join(input_images_path, keyframe_images_path) + "/*.txt")):
                    # print(video_path)
                    #
                    # txt_path = video_path.replace(input_videos_path, input_images_path).replace("mp4", "txt")
                    if os.path.exists(txt_path):
                        images_path_ = txt_path.replace(".txt", "")
                        ...
                        segment_index_list = extract_segment_frames_from_txt_file(None, txt_path)
                        # print("segment_frames_list: ", len(segment_frames_list))
                        segment_images_list, segment_frames_list = segment_images_by_segment_index(images_path_list, segment_index_list)
                        # print("segment_frames_list: ", len(segment_frames_list))
                        ...
                        for segment_frames, segment_images, segment_index in zip(segment_frames_list, segment_images_list, segment_index_list):
                            if len(segment_frames) >= 1:
                                frames_tensor = frames2tensor(segment_frames, fnum=1, target_size=(size_t, size_t), device=self.device)
                                ...
                                pass
                                # print(f"dont have keyframe in segment vid in {images_path_.split('/')[-1]}: {segment_index} --- len"
                                #       + f": {len(segment_frames)}")
